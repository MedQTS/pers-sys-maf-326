...
begin
  ...
  v_one_u_pct_global := coalesce((v_staking_config ->> 'global_1u_pct')::numeric, 0.015);

  if p_system_code = 'SYS_7' then
    v_one_u_pct_sys := (v_staking_config ->> 'system_7_1u_pct')::numeric;
  end if;
  ...
  if p_system_code = 'SYS_7' then
    ...
    v_base_pct := coalesce((v_staking_config ->> 'base_bankroll_pct')::numeric, 0.01);
